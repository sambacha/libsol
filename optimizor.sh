...
try_runs() {
    ...
    local contractSize
    # TODO change grep hardcode value of Auctioneer to take passable argument or something to accept new contract names
    contractSize=$(echo "$result" | grep $TARGET_CONTRACT | head -n 1 | awk -F'|' '{print $3}' | awk '{print $1}')
